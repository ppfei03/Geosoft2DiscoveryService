...
describe('filterMaxDate', () => {
  it('filters the correct elements for a valid give date', done => {
    const validDateTimeString = "2017-09";
    const reqObj = {filterResult: testArray, query: {maxDate: validDateTimeString}};

    maxDateFilter(reqObj).then(result => {
      expect(result.filterResult.length).toEqual(2); done();
    });
  });

  it('filters the correct elements for an invalid give date', done => {
    const inValidDateTimeString = "_";
    const reqObj = {filterResult: testArray, query: {maxDate: inValidDateTimeString}};
