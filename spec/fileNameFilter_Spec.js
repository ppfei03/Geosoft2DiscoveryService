...
describe('testSceneName', () => {
  ...
  it('returns false, if seachstring is not in string', done => {
    const sceneName = "S2A_MSIL1C_20161212T0823…GD_20161212T084403.SAFE";
    const search = ["0000"];
    let result = testSceneName(sceneName, search)
...
describe('fileNameFilter', () => {
  it('filters the correct elements for a matching search query', done => {
  ...
  it('filters no elements if search query does not contain a matching string', done => {
    ...
    fileNameFilter(reqObj).then(result => {
      expect(result.filterResult.length).toEqual(0); done();
    });
  });
